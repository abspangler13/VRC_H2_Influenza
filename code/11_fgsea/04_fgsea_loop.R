...
## 1. Read in data -----------------------------------------------------------
df_raw <- readRDS(file = here::here("analysis","data_objects","06_DE","de_rna_all_genes.rds")) #give it all the genes

seurat_raw <- readRDS(file = here::here("analysis","data_objects","05_clustering", "H2_Mem_G_clusters.rds"))
seurat_raw <- SetIdent(seurat_raw, value = "Cluster.res0.4")

clusters <- c("RM_2","AM1a_4","AM3_chronic_3","AM3_acute_1","AM2_0","IgM_5","AM1a_6")

## 2. Prepare background genes -----------------------------------------------
# Download gene sets .gmt files
#https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp
bg_path <- here::here("analysis","data_objects","11_fgsea","fgsea_dbs")  #Path to the downloaded fgsea databases/genesets
...
for(i in 1:length(clusters)) {
  ...
  seurat <- seurat_raw %>% filter(Cluster.res0.4 == clusters[i])
  ...
  # For GSEA
    ...
    saveRDS(rankings, file = here::here("analysis","data_objects","11_fgsea","new_de_collapse",paste0(clusters[i],"_gene_rank_stats.rds")))

    pdf(file = here::here("analysis","plots","11_fgsea","new_de_collapse",paste0(clusters[i],"_gene_rankings.pdf")))
    plot(rankings)
    ...
    for(j in 1:length(gmt_files)){
        ...
        print(paste0("significant up regulated pathways ",sum(GSEAres[, padj < 0.05 & ES > 0]))) ##print the number of significantly upregulated genesets
        print(paste0("significant down regulated pathways ",sum(GSEAres[, padj < 0.05 & ES < 0]))) ##print the number of significantly downregulated genesets
        sum(is.na(GSEAres$NES))

        saveRDS(GSEAres, file = here::here("analysis","data_objects","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],'_GSEAres.rds')))
        ## 6. Check results ------------------------------------------------------
        ...
        if(sum(GSEAres[, padj < 0.05 & ES > 0]) > 0){
            ...
            pdf(file = here::here("analysis","plots","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],'_gsea_top_pathways_up.pdf')), width = 20, height = 15)
            ...
            pdf(file = here::here("analysis","plots","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],'_gsea_top_pathways_up_plot.pdf')))
        ...
        if(sum(GSEAres[, padj < 0.05 & ES < 0]) > 0){
            ...
            pdf(file = here::here("analysis","plots","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],'_gsea_top_pathways_down.pdf')), width = 20, height = 15)
            plot(plotGseaTable(bg_genes[topPathwaysDown], stats = rankings, fgseaRes = GSEAres, gseaParam = 0.5))
            dev.off()

             # plot the significantly enriched pathway
            pdf(file = here::here("analysis","plots","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],'_gsea_top_pathways_down_plot.pdf')))
        ...
        collapsedPathways <- collapsePathways(
          ...
          saveRDS(collapsedPathways, file = here::here("analysis","data_objects","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],"_CollapsePathways.rds")))
          GSEAresSig <- GSEAres[order(pval)][padj < 0.05]
          GSEAresSig$parentPathway <- collapsedPathways$parentPathways
          saveRDS(GSEAresSig,file = here::here("analysis","data_objects","11_fgsea","new_de_collapse",paste0(clusters[i],"_",list.files(bg_path,pattern = '.gmt')[j],"_GSEAresCollapsed.rds")))
    }
