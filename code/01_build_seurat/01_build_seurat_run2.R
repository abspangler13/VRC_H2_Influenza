...
sample_info <- data.frame(
  sample_id = ### code removed because it contains PHI,
  cellranger_id = c("S1_and_S7_ALL_CUSTOM_label_ReRun",
                    "S2_and_S8_ALL_CUSTOM_label_ReRun"
                    ),
  vac_grp = c("3A",
              "3A")
)

sample_info$cellranger_raw_dir <- file.path(here::here("10XNxtSq_CITESeq_Exp15_210630_AAAJFJNM5","ver_020"),
                                        sample_info$cellranger_id,
                                        "raw_feature_bc_matrix")

sample_info$cellranger_filtered_dir <- file.path(here::here("10XNxtSq_CITESeq_Exp15_210630_AAAJFJNM5","ver_020"),
                                        sample_info$cellranger_id,
...
write.csv(sample_info, file = here::here("analysis","data_objects","01_build_seurat","sample_info_stats_run2.csv"))
...
#plot grob lists
pdf(file = here::here("analysis","plots","01_build_seurat","qc_b4_grobs_run2.pdf"))
lapply(QC.plots.b4,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","qc_after_grobs_run2.pdf"))
lapply(QC.plots.after,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","background_grobs_run2.pdf"))
marrangeGrob(background.plots, ncol = 2, nrow = 2)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_vln_grobs_run2.pdf"))
lapply(HTO.vln.plots, plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_ridge_grobs_run2.pdf"))
lapply(HTO.ridge.plots,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_scatter_grobs_run2.pdf"))
marrangeGrob(HTO.scatter.plots, ncol = 2, nrow = 2)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","rna_vln_grobs_run2.pdf"))
lapply(RNA.vln.plots, plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","rna_count_vln_grobs_run2.pdf"))
