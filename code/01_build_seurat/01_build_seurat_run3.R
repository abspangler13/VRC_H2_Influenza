...
sample_info$cellranger_raw_dir <- paste0("/hpcdata/vrc_vip/Abby/Experiment_316/10XNxtSq_CITESeq_Exp21Exp19MIXED_211020_BH3HN5DMXY/ver_010/",
                                        sample_info$cellranger_id,
                                        "/outs/raw_feature_bc_matrix")

sample_info$cellranger_filtered_dir <- paste0("/hpcdata/vrc_vip/Abby/Experiment_316/10XNxtSq_CITESeq_Exp21Exp19MIXED_211020_BH3HN5DMXY/ver_010/",
                                        sample_info$cellranger_id,
                                        "/outs/filtered_feature_bc_matrix")

sample_info$protein_csv_file = paste0("/hpcdata/vrc_vip/Abby/Experiment_316/analysis/data_objects/01_build_seurat/",
                             sample_info$sample_id,
                             paste0("/",sample_info$sample_id,"_sample_specific_oligos.csv"))

sample_info$log.txt = paste0("/hpcdata/vrc_vip/Abby/Experiment_316/analysis/data_objects/01_build_seurat/",
                                sample_info$sample_id,
                                paste0("/",sample_info$sample_id,"_log.txt"))

sample_info$run = "run3"
...
for (i in 1:nrow(sample_info)){
  ...
  oligoslist <- rownames(as.matrix(raw.data$Custom)) 
  #make csv of oligos
  ...
  probes.oligos[grep("SA_PE_0952",probes.oligos)] <- "SA_0972"
  probes.oligos[grep("H2_oligo",probes.oligos)] <- "H2_oligo3"
  ...
  HTO <- as.matrix(raw.data$Custom[hto.oligos,])
  message("HTO assay dim ",dim(HTO)[1]," ",dim(HTO)[2] )
  
  Probes <- as.matrix(raw.data$Custom[probes.oligos,])
  message("Probes assay dim ",dim(Probes)[1]," ",dim(Probes)[2] )
  
  Prot <- as.matrix(raw.data$Custom[prots.oligos,])
    #### remove CD-20
  ...
  source(file = here::here("analysis","code","DropCells_FUN.R"))
  seurat_pos <- DropCells(seurat, min.features = 100, min.hk = 50, max.mito = 15, min.count = 100, max.count = 50000, path.out = here::here("analysis","data_objects","01_build_seurat",sample_info$sample_id[i],"QC_stats.csv"))
...
write.csv(sample_info, file = here::here("analysis","data_objects","01_build_seurat","sample_info_stats_run3.csv"))

#save grob lists 
# saveRDS(QC.plots.b4, file = here::here("analysis","plots","01_build_seurat","qc_b4_grobs_run3.rds")) 
# saveRDS(QC.plots.after, file = here::here("analysis","plots","01_build_seurat","qc_after_grobs_run3.rds"))
# saveRDS(background.plots, file = here::here("analysis","plots","01_build_seurat","background_grobs_run3.rds"))
# saveRDS(HTO.vln.plots,file = here::here("analysis","plots","01_build_seurat","HTO_vln_grobs_run3.rds"))
# saveRDS(HTO.ridge.plots, file = here::here("analysis","plots","01_build_seurat","HTO_ridge_grobs_run3.rds"))
# saveRDS(HTO.scatter.plots, file = here::here("analysis","plots","01_build_seurat","HTO_scatter_grobs_run3.rds"))
# saveRDS(RNA.vln.plots, file = here::here("analysis","plots","01_build_seurat","rna_vln_grobs_run3.rds"))

#plot grob lists
pdf(file = here::here("analysis","plots","01_build_seurat","qc_b4_grobs_run3.pdf"))
lapply(QC.plots.b4,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","qc_after_grobs_run3.pdf"))
lapply(QC.plots.after,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","background_grobs_run3.pdf"))
marrangeGrob(background.plots, ncol = 2, nrow = 2)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_vln_grobs_run3.pdf"))
lapply(HTO.vln.plots, plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_ridge_grobs_run3.pdf"))
lapply(HTO.ridge.plots,plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","HTO_scatter_grobs_run3.pdf"))
marrangeGrob(HTO.scatter.plots, ncol = 2, nrow = 2)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","rna_vln_grobs_run3.pdf"))
lapply(RNA.vln.plots, plot)
dev.off()

pdf(file = here::here("analysis","plots","01_build_seurat","rna_count_vln_grobs_run3.pdf"))
